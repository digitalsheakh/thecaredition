 ...
 const [confirmedPrice, setConfirmedPrice] = useState(0);
  ...
  const columns: ColumnDef<BookingData>[] = [
    ...
    },
    {
      ...
      accessor: (booking) => (
        <div className="py-3 space-y-1">
          <div className="text-base font-bold text-gray-900 font-orbitron uppercase tracking-wider">
            {booking.vehicle.registrationNumber}
          </div>
          <div className="text-sm text-gray-700 font-rajdhani font-medium">
        ...
        <div className="py-3 space-y-2">
          <div className="space-y-1">
            {booking.services.map((service, i) => (
              <div key={i} className="bg-white border border-gray-200 px-3 py-2 rounded-lg shadow-sm">
                <span className="text-sm font-semibold text-gray-900 font-rajdhani">{service.name}</span>
              </div>
            ))}
          </div>
    ...
    },
    {
      ...
      accessor: (booking) => (
        <div className="py-3 text-center">
          <span className="inline-flex px-4 py-2 text-sm font-bold rounded-lg font-rajdhani uppercase tracking-wide shadow-sm bg-yellow-50 text-yellow-700">
            {booking.status}
          </span>
        </div>
      ),
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="px-6 py-6 border-b border-gray-200 bg-gradient-to-r from-red-50 to-red-100/50">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 font-orbitron uppercase tracking-wider">New Service Requests</h1>
            <p className="text-sm text-gray-600 mt-2 font-rajdhani">Manage incoming customer service requests and bookings</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        <SearchInput
          ...
          placeholder="Search by customer name, vehicle registration, or service type..."
        />
  ...
  <DialogContent className="border-gray-200 max-w-md">
    <DialogHeader>
      <DialogTitle className="text-gray-900 font-orbitron">Update Service Request Status</DialogTitle>
    </DialogHeader>
    
    <div className="space-y-4 py-4">
      <div>
        <label className="block text-sm font-medium mb-2 text-gray-700 font-rajdhani">Current Status</label>
      ...
      <div>
        <label className="block text-sm font-medium mb-2 text-gray-700 font-rajdhani">New Status</label>
            ...
            {statusOptions.map(status => (
              <SelectItem key={status} value={status}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-800 font-rajdhani">{status}</span>
                </div>
              </SelectItem>
            ))}
          </SelectContent>
      ...
      {/* Price Confirmation Field */}
      <div>
        <label className="block text-sm font-medium mb-2 text-gray-700 font-rajdhani">
          Confirm Service Price (£)
        </label>
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 font-rajdhani">£</span>
          <input
            type="number"
            defaultValue={confirmedPrice}
            onChange={(e) => setConfirmedPrice(Number(e.target.value))}
            placeholder="Enter confirmed price"
            className="pl-8 w-full rounded-md border border-gray-300 py-2 px-3 focus:ring-red-500 focus:border-red-500 font-rajdhani"
          />
        </div>
      </div>
    </div>
    ...
    <DialogFooter>
      <Button 
        variant="outline" 
        className="border-gray-300 text-gray-700 hover:bg-gray-50 font-rajdhani"
        onClick={() => {
          setIsModalOpen(false);
          setConfirmedPrice(0);
        }}
